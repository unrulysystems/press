...
| REQ-PUB-011                                   | `apps/web/src/publish/pagePathLocks.test.ts` deterministic lock order; `apps/web/src/publish/storage.test.ts` move/rollback/collision; `apps/web/src/db/schema.test.ts` redirect shape; `e2e/publish.spec.ts` guard, archive-reclaim, audit, and forced-failure rollback flow |
