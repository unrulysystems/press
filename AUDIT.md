...
**Suggested fix**: Provider tokens are no longer stored, so no further action there. Session-token hashing and column drops remain consciously declined (they require Better Auth adapter/schema overrides for marginal benefit). Session bearer tokens remain Better Auth's default unhashed values; continue treating DB/backups as secret-bearing and rotate sessions after restore exposure.
